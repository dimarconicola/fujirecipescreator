...
- NI-031 started with `calibration:tune:report` / `calibration:camera:tune:report`, adding per-axis directional tuning guidance and low-signal bootstrap detection to block false-confidence mapping updates.
...
| NI-031 | W8 | Rendering | 2026-04-14 | 2026-04-17 |  | in_progress | Added calibration tuning report CLI with per-axis actions and low-signal detection (`scripts/report-calibration-tuning.mjs`); next is applying and validating constant updates against non-bootstrap camera exports. |
