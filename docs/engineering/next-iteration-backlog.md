...
- NI-031 started with `calibration:tune:report` / `calibration:camera:tune:report`, adding per-axis directional tuning guidance and low-signal bootstrap detection to block false-confidence mapping updates; CI can now enforce this via `CAMERA_TUNING_SIGNAL_REQUIRED=true`.
...
| NI-031 | P1 | Tune control mapping functions (tone/chrome/grain/clarity) using calibration data and scene-class coverage. | Rendering | M | NI-030 | Mapping updates improve aggregate calibration metrics and preserve interactive performance budgets. | in_progress |
...
| NI-031 | W8 | Rendering | 2026-04-14 | 2026-04-17 |  | in_progress | Added calibration tuning report CLI with per-axis actions and low-signal detection (`scripts/report-calibration-tuning.mjs`) plus optional CI tuning-signal gate (`CAMERA_TUNING_SIGNAL_REQUIRED=true`); next is applying and validating constant updates against non-bootstrap camera exports. |
